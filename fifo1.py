...
def header( module_name ):
    ...
    V.module_header_end()
    V.iface_dprint( f'xx2fifo',               C.xx2fifo,               f'xx2fifo_pvld' )
    V.iface_dprint( f'fifo2xx',               C.fifo2xx,               f'fifo2xx_pvld' )
...
def make_tb_fifo1( name, module_name ):
    ...
    V.reg( f'xx2fifo_dat', C.fifo_w )
    V.reg( f'fifo2xx_dat_expected', C.fifo_w )
    V.always_at_posedge()
    P( f'    if ( !{V.reset_} ) begin' )
    P( f'        wr_cnt <= 0;' )
    P( f'        rd_cnt <= 0;' )
    P( f'        xx2fifo_dat <= 0;' )
    P( f'        fifo2xx_dat_expected <= 0;' )
    P( f'    end else begin' )
    P( f'        if ( xx2fifo_pvld && xx2fifo_prdy ) begin' )
    P( f'            xx2fifo_dat <= xx2fifo_dat + 1;' )
    P( f'            wr_cnt <= wr_cnt + 1;' )
    P( f'        end' )
    P( f'        if ( fifo2xx_pvld && fifo2xx_prdy ) begin' )
    P( f'            fifo2xx_dat_expected <= fifo2xx_dat_expected + 1;' )
    P( f'            rd_cnt <= rd_cnt + 1;' )
    P( f'        end' )
    ...
    P( f'    end' )
    P( f'end' )
    V.dassert( f'fifo2xx_pvld === 0 || fifo2xx_dat === fifo2xx_dat_expected', f'unexpected read data' )
